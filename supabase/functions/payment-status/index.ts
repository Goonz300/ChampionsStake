...
async function handler(ctx: EdgeContext): Promise<Response> {
  ...
  const { data: intent } = await supabase.from("payment_intents").select("*")
    .eq("provider_ref", query.providerRef).maybeSingle();
  if (!intent) {
    throw new NotFoundError(
      `No payment found for reference ${query.providerRef}.`,
    );
  }
  if (intent.user_id !== ctx.user!.id) {
    throw new AuthorizationError("This payment does not belong to you.");
  }
