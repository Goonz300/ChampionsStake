...
import { successResponse, noContentResponse } from "../_shared/response/index.ts";
import { ValidationError } from "../_shared/errors/index.ts";
import { updateChallengeSchema, updateChallenge, deleteChallenge } from "../_challenge/workflow.ts";

const pathSchema = z.object({ challengeId: z.string().uuid() });

async function handlePatch(ctx: EdgeContext, challengeId: string): Promise<Response> {
  requirePlayer(ctx.profile!);
  const body = validateBody(updateChallengeSchema, await parseJsonBody(ctx.request));
  await updateChallenge(challengeId, ctx.user!.id, body);
  return successResponse({ updated: true });
}

async function handleDelete(ctx: EdgeContext, challengeId: string): Promise<Response> {
  requirePlayer(ctx.profile!);
...
async function handler(ctx: EdgeContext): Promise<Response> {
  ...
  const { challengeId } = pathSchema.parse({ challengeId: url.searchParams.get("challengeId") });
  ...
  throw new ValidationError(`Unsupported method ${ctx.request.method}. Use PATCH to edit or DELETE to remove a draft.`);
}
