...
function assertStatus(
  ...
  if (!expected.includes(challenge.status)) {
    throw new ConflictError(
      `Cannot ${action} challenge ${challenge.id}: current status is "${challenge.status}", expected one of [${
        expected.join(", ")
      }].`,
    );
  }
...
export async function publishChallenge(
  ...
  if (challenge.creatorId !== callerId) {
    throw new AuthorizationError(
      "Only the creator may publish this challenge.",
    );
  }
  ...
  await recordChallengeEvent(challengeId, "ChallengeCreated", callerId, {
  ...
  await emit({
    ...
    payload: { challengeId, creatorId: callerId },
    emittedBy: "challenge-publish",
  });
  await emit({
    type: "EscrowLocked",
    payload: { challengeId, side: "creator" },
    emittedBy: "challenge-publish",
  });
}
...
export async function acceptChallenge(
  ...
  await supabase.from("challenges").update({ opponent_id: callerId }).eq(
    "id",
    challengeId,
  );
  ...
  await recordChallengeEvent(challengeId, "EscrowLocked", callerId, {
    side: "opponent",
  });
  await recordChallengeEvent(challengeId, "ChatOpened", null, {
  ...
  await emit({
    ...
    payload: { challengeId, opponentId: callerId },
    emittedBy: "challenge-accept",
  });
  await emit({
    type: "EscrowLocked",
    payload: { challengeId, side: "opponent" },
    emittedBy: "challenge-accept",
  });
  ...
  await emit({
    ...
    payload: { reason: "chat_opened", challengeId },
    emittedBy: "challenge-accept",
  });
}
...
export async function readyCheck(
  ...
  if (!(await isParticipant(challengeId, callerId))) {
    throw new AuthorizationError(
      "Only challenge participants may ready-check.",
    );
  }

  const supabase = getServiceRoleClient();
  const readyDeadline = new Date(
    Date.now() + READY_CHECK_WINDOW_MINUTES * 60 * 1000,
  ).toISOString();
  ...
  const bothReady = (participants ?? []).every((p) => p.ready_at !== null) &&
...
export async function startMatch(challengeId: string): Promise<void> {
  ...
  const countdownStartedAt = countdownEvent
    ? new Date(countdownEvent.created_at).getTime()
    : 0;
  const elapsedSeconds = (Date.now() - countdownStartedAt) / 1000;

  if (elapsedSeconds < COUNTDOWN_SECONDS) {
    throw new ConflictError(
      `Countdown has not yet elapsed for challenge ${challengeId} (${
        elapsedSeconds.toFixed(1)
      }s of ${COUNTDOWN_SECONDS}s).`,
    );
  }
...
export async function declareWinner(
  ...
  if (!(await isParticipant(challengeId, callerId))) {
    throw new AuthorizationError(
      "Only challenge participants may declare a winner.",
    );
  }
...
export async function releaseFunds(
  ...
  if (!(await isParticipant(challengeId, callerId))) {
    throw new AuthorizationError(
      "Only challenge participants may release funds.",
    );
  }
  if (challenge.winnerSubmittedBy === callerId) {
    throw new AuthorizationError(
      "The party who submitted the winning claim cannot release their own funds.",
    );
  }
  ...
  await recordChallengeEvent(challengeId, "FundsReleased", callerId, {
    winner_id: winnerId,
    fee_cents: feeCents,
  });
  await recordAudit({
  ...
  await emit({
    type: "FundsReleased",
    payload: { challengeId, winnerId, feeCents },
    emittedBy: "challenge-release",
  });
...
export async function moderatorResolveDispute(
  ...
  assertStatus(
    challenge,
    ["moderator_review"],
    "resolve via moderator decision for",
  );
 ...
 */
export async function moderatorVoidMatch(
  challengeId: string,
  moderatorId: string,
): Promise<void> {
  const challenge = await getChallengeOrThrow(challengeId);
  assertStatus(
    challenge,
    ["moderator_review"],
    "void via moderator decision for",
  );
  ...
  await recordChallengeEvent(challengeId, "ChallengeCancelled", moderatorId, {
    reason: "Voided by moderator decision",
  });
  await recordAudit({
    actorId: moderatorId,
    actorType: "moderator",
  ...
  await emit({
    type: "FundsReleased",
    payload: { challengeId, event: "MatchVoided" },
    emittedBy: "moderator-decision",
  });
}
...
export async function cancelChallenge(
  ...
  if (challenge.creatorId !== callerId) {
    throw new AuthorizationError(
      "Only the creator may cancel a challenge before it's accepted.",
    );
  }
