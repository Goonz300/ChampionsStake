...
const postBodySchema = z.discriminatedUnion("action", [
  z.object({ action: z.literal("freeze"), walletId: z.string().uuid(), reason: z.string().min(1) }),
...
async function handleGet(ctx: EdgeContext): Promise<Response> {
  ...
  if (query.view === "balance") return successResponse(await adminGetBalance(query.userId));
  if (query.view === "transactions") return successResponse(await adminGetTransactions(query.userId, query.limit, query.cursor));
  if (query.view === "ledger") return successResponse(await adminGetLedger(query.userId, query.limit));

  if (!query.from || !query.to) throw new ValidationError("from and to are required for the statement view.");
  const statement = await adminExportStatement(query.userId, query.from, query.to, query.format);
...
Deno.serve(withEdgeFunction({ functionName: "admin-wallets", auth: "required" }, handler));
