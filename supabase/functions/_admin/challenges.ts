...
import {
  archiveChallenge as challengeArchive,
  getChallengeTimeline as challengeTimeline,
 ...
 * override.
 */
export async function forceCancelChallenge(
  challengeId: string,
  adminId: string,
  reason: string,
): Promise<void> {
  ...
  await recordChallengeEvent(challengeId, "ChallengeCancelled", adminId, {
    reason,
...
export async function adminBrowseChallenges(
  filters: { status?: string; gameId?: string; limit: number; cursor?: string },
) {
  const supabase = getServiceRoleClient();
  let query = supabase.from("challenges").select("*").order("created_at", {
    ascending: false,
  }).limit(filters.limit);
