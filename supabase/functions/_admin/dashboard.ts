...
export async function getDashboardMetrics(): Promise<DashboardMetrics> {
  ...
  const [
    ...
    supabase.from("challenges").select("id", { count: "exact", head: true }).eq(
      "status",
      "live",
    ),
    supabase
    ...
    supabase.from("disputes").select("id", { count: "exact", head: true }).in(
      "status",
      ["open", "under_review"],
    ),
  ]);
  ...
  const walletVolumeCentsLast24h = (recentTransactions ?? []).reduce(
    (sum, t) => sum + t.amount_cents,
    0,
  );

  const { data: lockedEscrow } = await supabase.from("escrow_accounts").select(
    "total_locked_cents",
  ).eq("status", "locked");
  const escrowVolumeCentsLocked = (lockedEscrow ?? []).reduce(
    (sum, e) => sum + e.total_locked_cents,
    0,
  );

  const { data: storageAgg } = await supabase.from("file_uploads").select(
    "file_size_bytes",
  ).neq("status", "deleted");
  const storageUsageBytes = (storageAgg ?? []).reduce(
    (sum, f) => sum + f.file_size_bytes,
    0,
  );
