...
import { getBalance, freezeWallet, unfreezeWallet } from "../_wallet/service.ts";
import { listTransactions, getWalletByUserIdOrThrow } from "../_wallet/repository.ts";
...
export async function adminGetTransactions(userId: string, limit: number, cursor?: string) {
...
export async function adminExportStatement(userId: string, from: string, to: string, format: "json" | "csv") {
  const wallet = await getWalletByUserIdOrThrow(userId);
