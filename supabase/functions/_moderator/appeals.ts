...
import { ConflictError, AuthorizationError } from "../_shared/errors/index.ts";
...
export async function fileAppeal(disputeId: string, filedBy: string): Promise<void> {
  const dispute = await getDisputeOrThrow(disputeId);

  if (dispute.status !== "resolved") {
    throw new ConflictError(`Dispute ${disputeId} has no decision to appeal (status: ${dispute.status}).`);
  }
  if (dispute.appealFiledAt) {
    throw new ConflictError("One appeal has already been filed for this dispute (Business Rules §9: one per dispute).");
  }
  if (!dispute.decidedAt || Date.now() - new Date(dispute.decidedAt).getTime() > APPEAL_WINDOW_HOURS * 60 * 60 * 1000) {
    throw new ConflictError(`The ${APPEAL_WINDOW_HOURS}-hour appeal window has passed.`);
  }
  ...
  await updateDispute(disputeId, {
    ...
    appeal_deadline_at: new Date(now.getTime() + APPEAL_WINDOW_HOURS * 60 * 60 * 1000).toISOString(),
  });
  ...
  await emit({ type: "DisputeOpened", payload: { disputeId, event: "AppealFiled" }, emittedBy: "moderator-appeal" });
}
...
export async function assignAppealReviewer(disputeId: string, reviewerId: string, actorId: string): Promise<void> {
  const supabase = getServiceRoleClient();
  const { data: reviewer } = await supabase.from("profiles").select("role, status").eq("id", reviewerId).maybeSingle();
  if (!reviewer || reviewer.role !== "administrator" || reviewer.status !== "active") {
    throw new AuthorizationError("Appeal reviewers must be an active administrator (Business Rules §9: senior moderator/admin).");
  }
...
export async function decideAppeal(
  ...
  if (dispute.status !== "appealed") {
    throw new ConflictError(`Dispute ${disputeId} is not currently under appeal (status: ${dispute.status}).`);
  }
  ...
  await recordAudit({
    ...
    targetTable: "disputes",
    targetId: disputeId,
    metadata: { resolution, rationale, note: "Verdict recorded; any fund reversal (if the verdict differs from the original decision) requires a separate ADMIN-001 wallet adjustment." },
  });

  await emit({ type: "ModeratorDecisionRecorded", payload: { disputeId, event: "AppealDecided", resolution }, emittedBy: "moderator-appeal" });
}
