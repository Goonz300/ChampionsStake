// supabase/functions/ai-reputation-engine/index.ts
// Scheduled sweep (or admin-triggered), same dual admin-or-scheduled-secret
// pattern as ai-trust-score/ai-fraud-scan/ai-ip-intelligence.
...
import { sweepReputationScores } from "../_ai/reputation-engine.ts";
...
async function handler(ctx: EdgeContext): Promise<Response> {
  ...
  const result = await sweepReputationScores();
...
Deno.serve(
  withEdgeFunction(
    {
      functionName: "ai-reputation-engine",
      auth: "optional",
      rateLimit: (ctx) => ({
        key: `ai-reputation-engine:${ctx.user?.id ?? "scheduled"}`,
        windowSeconds: 60,
        maxRequests: 5,
      }),
    },
