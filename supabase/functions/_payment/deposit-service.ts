...
import { ValidationError, ConflictError } from "../_shared/errors/index.ts";
...
export async function initializeDeposit(userId: string, amountCents: number, idempotencyKey: string) {
  ...
  const { data: existingIntent } = await supabase
    ...
    .maybeSingle();
  if (existingIntent) {
    throw new ConflictError(`A deposit with idempotency key ${idempotencyKey} already exists (status: ${existingIntent.status}).`);
  }

  const { data: authUser } = await supabase.auth.admin.getUserById(userId);
  const email = authUser?.user?.email;
  const { data: wallet } = await supabase.from("wallets").select("id").eq("user_id", userId).single();
  if (!email || !wallet) throw new ValidationError("User not found, has no email on file, or has no wallet.");

  const provider = await getActiveProvider();
  const result = await provider.initializeTransaction({ amountCents, currency: "NGN", userId, email, idempotencyKey });
  ...
  await emit({
    ...
    payload: { userId, event: "DepositInitiated", providerRef: result.providerRef },
 ...
 */
export async function verifyAndCompleteDeposit(providerRef: string): Promise<{ status: string }> {
  const supabase = getServiceRoleClient();
  const provider = await getActiveProvider();
  ...
  const { data: intent } = await supabase
    ...
    .eq("kind", "deposit")
    .maybeSingle();
  if (!intent) throw new ValidationError(`No payment intent found for provider reference ${providerRef}.`);
  ...
  if (verification.status !== "success") {
    ...
    await emit({ type: "TransactionFailed", payload: { providerRef, event: "DepositFailed" }, emittedBy: "payment-verify" });
  ...
  if (verification.amountCents !== intent.amount_cents) {
    ...
    await recordAudit({
      ...
      targetTable: "payment_intents",
      targetId: intent.id,
      metadata: { expected: intent.amount_cents, providerConfirmed: verification.amountCents },
    });
    throw new ValidationError(
  ...
  const result = await completeDeposit(intent.wallet_id, intent.amount_cents, provider.name, providerRef, intent.idempotency_key);
